...
{   
    ...
    public function destroy(Request $request) {
        $tour_badges = TourBadge::findOrFail($request->id);

        $upload_image = public_path('assets/img/tour_badges/') . $tour_badges->id . '/' . $tour_badges->featured_image;

        if($upload_image) {
             @unlink($upload_image);
        }

        $remove = $tour_badges->delete();
